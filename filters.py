...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    the
    user's options at the command line. Each one corresponds to a different
    type
    ...
    # check the arguments passed into this method if they are not None,
    # and if so, append a Filter from the config dict
    for arg, val in locals().items():
        if (val is not None and arg in config):
            filters.append(config[arg])
